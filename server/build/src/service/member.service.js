...
Member.signin = (req, res) => {
    ...
    sql.query("SELECT * FROM member WHERE email=?", req.body.email, (err, rows) => {
...
Member.findAll = (req, res) => {
    sql.query("SELECT * FROM member", (error, rows) => {
        try {
            if (error) {
                res.status(500).json({
                    ok: false,
...
Member.findOne = (req, res) => {
    ...
    sql.query("SELECT * FROM member WHERE ?", req.params, (error, rows) => {
        try {
            if (error) {
                res.status(500).json({
                    ok: false,
...
Member.create = (req, res) => {
    ...
    bcrypt.hash(pw, saltRounds, (err, hash) => {
        req.body.pw = hash;
        sql.query("INSERT INTO member SET ?", req.body, (error, rows) => {
            try {
                if (error) {
                    res.status(500).json({
                        ok: false,
...
Member.update = (req, res) => {
    ...
    bcrypt.hash(pw, saltRounds, (err, hash) => {
        ...
        sql.query("UPDATE member SET ? WHERE ?", [req.body, req.params], (error, rows) => {
            try {
                if (error) {
                    res.status(400).json({
                        ok: false,
...
Member.delete = (req, res) => {
    sql.query("DELETE FROM member WHERE ?", req.params, (error, rows) => {
        try {
            if (error) {
                res.status(400).json({
                    ok: false,
